...
const BookModalCreate = (props) => {
    ...
    const onFinish = async (values) => {
        ...
        const { mainText, author, category, price, quantity, sold } = values;
        const thumbnail = dataThumbnail[0].name;
        const slider = dataSlider.map((item) => item.name);
        setIsSubmit(true);
        const res = await callCreateABook(thumbnail, slider, mainText, author, category, price, quantity, sold);
        if (res && res.data) {
